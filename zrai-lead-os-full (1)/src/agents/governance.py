...
class GovernanceAgent:
    ...
    def process(self, state: LeadGraphState) -> LeadGraphState:
        ...
        """
        self._logger.info(f"Governance check for lead {state.lead_id}")
        
        # Check if lead should be skipped
        if self.dnc_manager.is_on_dnc(state.lead_id):
            state.should_skip_outreach = True
            state.metadata["skip_reason"] = "DNC"
        
        # Check cool-down
        cool_down = self.rate_limiter.check_cool_down(state.lead_id)
        if cool_down:
            state.should_skip_outreach = True
            state.metadata["skip_reason"] = cool_down
