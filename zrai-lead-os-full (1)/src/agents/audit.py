...
class AuditAgent(BaseAgent, CircuitBreakerMixin):
    ...
    def process(self, state: LeadGraphState) -> LeadGraphState:
        ...
        should_audit = False
        if state.scoring and state.scoring.final_score >= self.SCORE_THRESHOLD:
            should_audit = True
        elif state.scoring and state.scoring.lead_tier == "A":
            should_audit = True
        elif state.intent and (state.intent.intent_score + state.intent.leak_score) / 2 >= self.SCORE_THRESHOLD:
            should_audit = True
        
        if not should_audit:
            self._logger.info(f"Lead {state.lead.lead_id} below audit threshold, skipping")
            state.should_skip_audit = True
            return state
        ...
        landing_page_url = state.lead.landing_page_url or state.lead.website
        ...
        try:
            ...
            audit_bullets = self._generate_audit_bullets(state.lead, extraction_data)
    ...
    def _save_proof(self, proof: ProofArtifact) -> None:
        """Save proof artifact to database."""
        data = proof.model_dump()
        data["lead_id"] = str(proof.lead_id)
        data["generated_at"] = datetime.utcnow().isoformat()
        
        # Convert audit bullets to JSON-serializable format
        data["audit_bullets"] = [
            {
                "type": b.type,
                "evidence": b.evidence,
