...
- **Firecrawl_Client**: Web scraping service for LLM-ready structured data extraction
- **Scraper_Router**: Intelligent routing layer that selects optimal scraping provider per task
- **Command**: LangGraph control flow primitive combining state updates with routing decisions
- **Send**: LangGraph primitive for fan-out parallel processing of multiple items
- **Batch_Lead_State**: State schema for parallel lead processing with reducer aggregation
...
### Requirement 24: Firecrawl Integration for Structured Extraction

**User Story:** As a system operator, I want to use Firecrawl for structured data extraction, so that I can get LLM-ready business information without complex parsing and with schema-based output.

#### Acceptance Criteria

1. WHEN the Firecrawl_Client initializes, THE Lead_OS SHALL load the Firecrawl API key from configuration or environment variables
2. WHEN extracting business information, THE Firecrawl_Client SHALL use schema-based extraction to return structured JSON with business_name, phone, email, booking_url, services, business_hours, has_online_booking, and has_contact_form
3. WHEN performing interactive extraction, THE Firecrawl_Client SHALL execute page actions including wait, scroll, and click before capturing data
4. WHEN extracting data, THE Firecrawl_Client SHALL return content in markdown format optimized for LLM consumption
5. WHEN batch scraping is needed, THE Firecrawl_Client SHALL process multiple URLs efficiently in a single request
6. WHEN the Firecrawl_Client encounters errors, THE Lead_OS SHALL fall back to alternative scraping providers
7. WHEN capturing proof artifacts, THE Firecrawl_Client SHALL capture screenshots after executing configured page interactions

### Requirement 25: Hybrid Scraping Strategy

**User Story:** As a cost-conscious operator, I want the system to route scraping requests to the optimal provider, so that I minimize costs while maximizing data quality and reliability.

#### Acceptance Criteria

1. WHEN the Scraper_Router receives a scraping request, THE Lead_OS SHALL evaluate the task_type to determine the optimal provider
2. WHEN task_type is bulk_discovery, THE Scraper_Router SHALL route to Apify for Google Maps and Meta Ads scraping
3. WHEN task_type is structured_extraction, THE Scraper_Router SHALL route to Firecrawl for LLM-structured data extraction
4. WHEN task_type is interactive_audit, THE Scraper_Router SHALL route to Firecrawl with page actions for proof generation
5. WHEN task_type is website_crawl, THE Scraper_Router SHALL route to Apify website-content-crawler with LLM-optimized settings
6. WHEN a provider fails, THE Scraper_Router SHALL automatically retry with an alternative provider
7. WHEN routing decisions are made, THE Scraper_Router SHALL log the selected provider and reasoning for observability
8. WHEN caching is enabled, THE Scraper_Router SHALL check cache before making external requests and cache results with configurable TTL

### Requirement 26: Command-Based Graph Routing

**User Story:** As a system engineer, I want combined state updates and routing decisions using LangGraph Commands, so that the graph logic is cleaner, more maintainable, and reduces conditional edge complexity.

#### Acceptance Criteria

1. WHEN a graph node completes processing, THE Graph_Orchestrator SHALL use Command objects to combine state updates with routing decisions
2. WHEN the Scoring_Agent assigns tier C, THE Graph_Orchestrator SHALL return a Command that updates is_disqualified to true and routes to the end node
3. WHEN the Scoring_Agent assigns tier A or B, THE Graph_Orchestrator SHALL return a Command that updates scoring data and routes to the outreach node
4. WHEN the Intent_Agent detects high leak_score, THE Graph_Orchestrator SHALL return a Command that routes to the audit node
5. WHEN the Intent_Agent detects low leak_score, THE Graph_Orchestrator SHALL return a Command that routes to the scoring node bypassing audit
6. WHEN using Commands, THE Graph_Orchestrator SHALL eliminate separate conditional edge functions where possible
7. WHEN a Command specifies multiple goto targets, THE Graph_Orchestrator SHALL support conditional routing within the Command

### Requirement 27: Parallel Lead Processing

**User Story:** As a system operator, I want discovered leads to be processed in parallel using map-reduce patterns, so that the pipeline throughput is maximized and processing time is minimized.

#### Acceptance Criteria

1. WHEN the Discovery_Agent returns multiple leads, THE Graph_Orchestrator SHALL use Send primitives to fan-out leads to parallel enrichment
2. WHEN parallel enrichment completes, THE Graph_Orchestrator SHALL use annotated reducers to aggregate results back into Batch_Lead_State
3. WHEN enriched leads are ready, THE Graph_Orchestrator SHALL use Send primitives to fan-out leads to parallel scoring
4. WHEN parallel scoring completes, THE Graph_Orchestrator SHALL aggregate scored leads using the configured reducer function
5. WHEN configuring parallelism, THE Lead_OS SHALL load max_parallel_leads from configuration to limit concurrent processing
6. WHEN a parallel branch fails, THE Graph_Orchestrator SHALL isolate the failure and continue processing other branches
7. WHEN all parallel branches complete, THE Graph_Orchestrator SHALL invoke an aggregate node to consolidate results before proceeding
8. WHEN using parallel processing, THE Lead_OS SHALL track per-branch metrics including latency, success_rate, and error_counts
