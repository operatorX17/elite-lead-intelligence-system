...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    try {
      ...
      return validationError('intent', { message: 'Invalid JSON body' }).toResponse();
    }
    ...
    if (!backendResponse.ok) {
      ...
      return backendError('intent', errorData.message).toResponse();
    }
    ...
    const response: IntentResponse = {
      ...
      data: {
        ...
        revenue_leak_score: data.revenue_leak_score || 0,
      },
    };
    ...
    return backendError('intent', error instanceof Error ? error.message : 'Unknown error').toResponse();
  }
}
