...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (!backendResponse.ok) {
      ...
      return backendError('run', errorData.message).toResponse();
    }
    ...
    return backendError('run', error instanceof Error ? error.message : 'Unknown error').toResponse();
  }
}
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    if (!runId || runId === 'run') {
      ...
      const data = await backendResponse.json();
      return Response.json({ success: true, data: data.runs || [] }, { status: 200 });
    }

    // Get specific run
    const backendResponse = await fetch(`${ZRAI_BACKEND_URL}/api/v1/run/${runId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'X-User-ID': session.user.id,
