...
class EnrichmentAgent(BaseAgent, CircuitBreakerMixin):
    ...
    def process(self, state: LeadGraphState) -> LeadGraphState:
        ...
        # Extract decision maker info from business name and live website copy.
        decision_maker = self._extract_decision_maker(lead, website_context.get("raw_content", ""))
    ...
    def _extract_decision_maker(self, lead: Dict[str, Any], raw_content: str = "") -> Dict[str, str]:
        """
        ...
        if token_match:
            ...
            if potential and potential.lower() not in stopwords:
                decision_maker["name"] = potential

        if raw_content:
            doctor_patterns = [
                r"\bDr\.?\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})",
                r"\bFounder[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})",
                r"\bDirector[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})",
            ]
            for pattern in doctor_patterns:
                match = re.search(pattern, raw_content)
                if match:
                    decision_maker["name"] = match.group(1).strip()
                    break
