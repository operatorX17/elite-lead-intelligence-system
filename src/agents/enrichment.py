...
class EnrichmentAgent(BaseAgent, CircuitBreakerMixin):
    ...
    def process(self, state: LeadGraphState) -> LeadGraphState:
        ...
        tech_signals = {}
        if lead.get("website"):
            tech_signals = self._extract_tech_signals(lead["website"])
        
        # Extract decision maker info
        decision_maker = self._extract_decision_maker(lead)
    ...
    def _extract_decision_maker(self, lead: Dict[str, Any]) -> Dict[str, str]:
        """
        ...
        name_match = re.match(r"^([A-Z][a-z]+(?:'s)?)\s", business_name)
        if name_match:
