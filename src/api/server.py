...
def build_signal_facts(
    ...
    email_contacts = list(lead_data.get("emails_found") or []) + list(
        enrichment_payload.get("validated_emails") or []
    )
    recommended_channel = "email" if email_contacts else "phone" if phone_numbers else None
    if not recommended_channel and whatsapp_detected:
...
async def get_lead(
    ...
    try:
        ...
        processed_details = get_processed_details_for_lead(db, lead_id)
