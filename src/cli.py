        ...
        with click.progressbar(leads, label="Dry run") as bar:
            for lead in bar:
                try:
                    ...
                    # Handle both dict and LeadGraphState results
                    if isinstance(result, dict):
                        current_stage = result.get("current_stage", "unknown")
                        scoring = result.get("scoring")
                        tier = scoring.get("lead_tier") if isinstance(scoring, dict) else (scoring.lead_tier if scoring else None)
                        score = scoring.get("final_score") if isinstance(scoring, dict) else (scoring.final_score if scoring else None)
                    else:
