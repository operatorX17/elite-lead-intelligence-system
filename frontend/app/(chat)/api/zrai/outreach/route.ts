...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    try {
      ...
      return validationError('outreach', { message: 'Invalid JSON body' }).toResponse();
    }
    ...
    if (!backendResponse.ok) {
      ...
      return backendError('outreach', errorData.message).toResponse();
    }
    ...
    const response: OutreachResponse = {
      ...
      data: {
        ...
        sent: body.action === 'send' ? data.sent : undefined,
      },
    };
    ...
    return backendError('outreach', error instanceof Error ? error.message : 'Unknown error').toResponse();
  }
}
