...
const normalizeCircuitBreakerState = (
  value: unknown
): CircuitBreakerState => {
  const normalized = String(value).toLowerCase();

  if (normalized === "open" || normalized === "half_open") {
    return normalized;
  }
...
export async function GET(_request: Request): Promise<Response> {
  try {
    ...
    const backendResponse = await fetch(
      `${ZRAI_BACKEND_URL}/api/v1/governance`,
      {
        method: "GET",
        headers: {
    ...
    const response: GovernanceResponse = {
      ...
      data: {
        ...
        circuit_breakers: Object.fromEntries(
          Object.entries(data.circuit_breakers || {}).map(([key, value]) => [
            key,
            normalizeCircuitBreakerState(value),
          ])
        ),
